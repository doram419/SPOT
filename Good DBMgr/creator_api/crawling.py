...
class CrawlingModule:
    ...
    def on_crawl(self):
        ...
        if len(keyword) > 1 and len(region) > 1:
            ...
            self.parent.after(100, lambda: self.start_crawling(keyword, region, mode))
        else:
            self.status_module.update_status("검색어 또는 지역이 누락되었습니다")
    ...
    def crawl_thread(self, keyword: str, region: str, mode: str):
        try:
            ...
            self.parent.after(0, lambda: self.crawl_button.config(state='normal'))
